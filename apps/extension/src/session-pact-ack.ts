...
import { formatSessionCapDuration } from '@tiltcheck/shared';
...
export function maybeShowSessionPactAck(ctx: SessionPactAckContext): void {
  ...
  const siteLabel = document.title.trim() || window.location.hostname;
  ...
  const configBlock = hasProtection
    ? `
      <ul style="margin:1.25rem 0 0;padding:0;list-style:none;text-align:left;max-width:28rem;width:100%">
        ...
        <li style="padding:.65rem 0;border-bottom:1px solid rgba(52,67,90,.6);font-size:clamp(.95rem,2vw,1.05rem);color:#e5e7eb">
          <span style="display:block;font:700 10px/1 ui-monospace,monospace;letter-spacing:.12em;text-transform:uppercase;color:#6b7280;margin-bottom:.25rem">Exit line</span>
          ${
            ctx.capArmed
              ? `${formatSessionCapDuration(ctx.cap.durationMinutes)} lockout · ${lockoutLabel(ctx.cap.lockoutStyle)}${ctx.cap.snoozeEnabled ? ' · one snooze allowed' : ''}`
              : 'Not set — pick a cap in TiltCheck → Settings'
          }
        </li>
    ...
    : `
      <p style="margin:1.25rem 0 0;max-width:28rem;font-size:clamp(1rem,2.2vw,1.15rem);color:#b8c5d0;line-height:1.55">
        Open the TiltCheck panel → <strong style="color:#e5e7eb">Settings</strong> to set your exit line and game blocks, then start playing.
      </p>
    `;

  const headline = hasProtection ? 'Your rules are armed for this casino' : 'Set your exit line before you bet';

  const leadCopy = hasProtection
    ? `These are the limits past-you chose. TiltCheck enforces them on casino tabs like this one — not email, not social, not random sites. Ack once; we won't ask again until you close this tab.`
    : `You're on a casino site (${escapeHtml(siteLabel)}). Arm an exit line and game blocks in Settings so TiltCheck can step in when tilt hits.`;
  ...
  root.innerHTML = `
    ...
    <div id="tc-pact-ack-panel">
      <p style="margin:0 0 1rem;font:700 11px/1 ui-monospace,monospace;letter-spacing:.18em;text-transform:uppercase;color:#5eead4">TC · CASINO SESSION</p>
      ...
      <label id="tc-pact-ack-check">
        ...
        <span>I know what's armed. When my line fires, I stop — I don't argue with past-me.</span>
      </label>
      <button type="button" id="tc-pact-ack-btn" disabled>Enter casino session</button>
    </div>
  `;
